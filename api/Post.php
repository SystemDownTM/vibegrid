...
if (
    strpos($Username, '#') !== false or strpos($Username, "'") !== false or strpos($Username, "\"") !== false or
...
if (
    strpos($Password, '#') !== false or strpos($Password, "'") !== false or strpos($Password, "\"") !== false or
...
if ($query) { // if query was ok
    while ($fetch = mysqli_fetch_assoc($query)) {
        if (trim($Password) == trim($fetch['Password']) && $Username == md5($fetch['Username'])) {
            Run();
            $query_sen = "SELECT * FROM posts";
            $query = mysqli_query($conn, $query_sen);
            $user_id = $fetch["id"];
            $post_text = htmlspecialchars($_GET["Text"]);
            $sql = "INSERT INTO posts (user_id, content) VALUES ('$user_id', '$post_text')";
